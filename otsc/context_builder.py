...
For screen readings with code_blocks, use only those mapped regions for observed_files. Remove exactly the declared gutter_separator and its editor number from each raw_text line; preserve every remaining indentation character, blank line and source comment. Derive first_line from the visible absolute numbers (including the offset for a sub-excerpt); for unnumbered code use first_line=null. Empty code_blocks means no source region is established. Never join different blocks into one file or infer changes from a previous proposal.
