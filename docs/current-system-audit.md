...
| OCR regions | `gui_section_boxes.py` and `ocr_pair_screenshot_blocks` detect panels and OCR blocks | Retain while unifying the workflow; replace only for a demonstrated quality or latency problem |
...
4. **The UI is coupled to content production.** Fixed character counts, page lists, and global section arrays make resizing and interaction fragile. Refactor rendering within the existing Python/AppKit application, using ordinary selectable and scrollable controls. The existing native window does not need a replacement UI framework.
