...
_is_running, _awaiting_second, _results_shown, _interactive_mode = threading.Event(), False, False, False
_status_label, _view_mode, _last_sections, _full_sections = None, 0, None, None
_diagram_image_path = None
_PAGES = [
...
def generate_diagram_async(data: ProductAssistantResponse):
    global _diagram_image_path
    try:
        client = genai.Client()
        response = client.models.generate_content(model="gemini-3-pro-image-preview", contents=[build_diagram_prompt(data)])
        for part in response.parts:
            if part.inline_data is not None:
                ts = time.strftime("%Y%m%d_%H%M%S")
                path = os.path.abspath(f"diagram_{ts}.png")
                part.as_image().save(path)
                _diagram_image_path = path
                if _view_mode == len(_PAGES): AppHelper.callAfter(_show_diagram_view)
                return
...
def _get_page_sections(sections: List[Tuple[str, List[str]]], page: int) -> List[Tuple[str, List[str]]]:
    if page >= len(_PAGES): return []
    page_titles = set(_PAGES[page])
    return [s for s in sections if s[0] in page_titles]

def _show_diagram_view():
    for sv in list(window.contentView().subviews()): sv.removeFromSuperview()
    frame = window.frame()
    if _diagram_image_path and os.path.exists(_diagram_image_path):
        img = NSImage.alloc().initWithContentsOfFile_(_diagram_image_path)
        if img:
            ...
            _update_status("Diagram (4/4)")
            return
    _place_label("Diagram generating...", 20, frame.size.height - 50, 400, TITLE_HEIGHT, TITLE_FONT, _mk_color(60, 60, 140))
    _update_status("Diagram (4/4) - generating...")

def _rerender_current_view():
    num_pages = len(_PAGES)
    if _view_mode < num_pages:
        if _full_sections:
            page_sections = _get_page_sections(_full_sections, _view_mode)
            AppHelper.callAfter(_update_overlay_sections, page_sections)
            AppHelper.callAfter(_update_status, f"Page {_view_mode + 1}/{num_pages + 1}")
    else:
...
def run_pipeline():
    global _results_shown, _diagram_image_path, _view_mode
    _diagram_image_path = None
    _view_mode = 0
    try:
        ui_update_sections([("Status", ["Generating response..."])] + cheat_sheet)
        try: shot = take_screenshot()
        except Exception as e: ui_update([f"Screenshot error: {e}"]); return
        data = call_anthropic_product(ocr_with_tesseract(shot), image_path=shot)
        if data:
            sections = format_structured_for_overlay_sections(data)
            def _render():
                global _last_sections, _full_sections
                _full_sections = _last_sections = sections
                _update_overlay_sections(sections); _update_status("All sections")
        ...
        else: ui_update(["(No structured response)"]); _results_shown = False
    except Exception as e: ui_update([f"Pipeline error: {e}"]); _results_shown = False
...
def _apply_interactive_mode():
    ...
    if _interactive_mode:
        ...
        ui_update(['Interactive mode OFF. Release "63" twice to run. Press F2 to enable.'])
...
_full_sections = _last_sections = cheat_sheet; ui_update_sections(cheat_sheet)
...
def on_release(key):
    global _awaiting_second, _results_shown, _view_mode
    try:
        ...
        if str(vk) == "63":
            if not _awaiting_second: _awaiting_second = True
                ...
                if _results_shown: _results_shown = False; _view_mode = 0; ui_update_sections(cheat_sheet); return
...
_last_note50_ts, _NOTE50_DEBOUNCE_SEC = 0.0, 0.15
...
def on_midi(msg):
    try:
        ...
        if mtype == "note_on" and note == 50 and (vel is None or vel > 0):
            ...
            now = time.time()
            if now - _last_note50_ts < _NOTE50_DEBOUNCE_SEC: return
            _last_note50_ts = now
            def _toggle():
                global _view_mode
                _view_mode = (_view_mode + 1) % (len(_PAGES) + 1)
